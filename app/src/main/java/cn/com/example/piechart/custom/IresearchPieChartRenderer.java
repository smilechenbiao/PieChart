 ...
 * Time: 11:25
 */
public class IresearchPieChartRenderer extends DataRenderer {

    protected IresearchPieChart mChart;
    ...
    public IresearchPieChartRenderer(IresearchPieChart chart, ChartAnimator animator,
            ...
            for (int j = 0; j < entryCount; j++) {
               ...
               /* if (value1 < 5) {
                ...
                if (drawXOutside || drawYOutside) {
                    ...
                    float LabelLength = mEntryLabelsPaint.measureText(entry.getLabel()); //计算文字的长度
                    float LineLength = 0F;
                    if (transformedAngle % 360.0 >= 90.0 && transformedAngle % 360.0 <= 270.0) {
                        ...
                        labelPty = pt2y;
                        LineLength = labelPtx - LabelLength; // 第二条线 x 起始绘制位置加上文字的长度
                    } else {
                        pt2x = pt1x + polyline2Width;
                        pt2y = pt1y;
                    ...
                    if (dataSet.getValueLineColor() != ColorTemplate.COLOR_NONE) {
                        ...
                        c.drawLine(pt1x, pt1y, LineLength, pt2y, mValueLinePaint);
                    }
                    ...
                    if (drawXOutside && drawYOutside) {
                       ...
                       /* if (value1 < 5) {
                        ...
                        if (j < data.getEntryCount() && entry.getLabel() != null) {
                            ...
                            /**通过图表角度绘制上下边文字显示位置*/
                            if (transformedAngle % 360.0 >= 180.0 && transformedAngle % 360.0 <= 360.0) {
                                /**绘制上面*/
                                drawEntryLabel(c, entry.getLabel(), labelPtx, labelPty - lineHeight / 2);
                            } else {
                                /**绘制下面*/
                                drawEntryLabel(c, entry.getLabel(), labelPtx, labelPty + lineHeight);
                            }
